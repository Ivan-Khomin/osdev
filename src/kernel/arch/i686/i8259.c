...
{
    // Mask everything
    i8259_SetMask(0xFFFFF);

    // initalization control word 1
    ...
    // initalization control word 2 - the offset
    i686_outb(PIC1_DATA_PORT, offsetPic1);
    i686_iowait();
    i686_outb(PIC1_DATA_PORT, offsetPic2);
    i686_iowait();

    // initalization control word 3
    i686_outb(PIC1_DATA_PORT, 0x4);             // tell PIC1 that it has a slave at IRQ2
    i686_iowait();
    i686_outb(PIC2_DATA_PORT, 0x2);             // tell PIC2 its cascade identity
    i686_iowait();

    // initalization control word 4
