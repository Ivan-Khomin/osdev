...
mkfs.fat -n "INVIS UA OS" $TARGET_PARTITION >/dev/null
...
# copy files
echo "Copying files to ${TARGET_PARTITION} (mounted on /tmp/invisos)..."
mkdir -p /tmp/invisos
mount ${TARGET_PARTITION} /tmp/invisos
cp ${BUILD_DIR}/kernel.bin /tmp/invisos
cp test.txt /tmp/invisos
mkdir /tmp/invisos/test
cp test.txt /tmp/invisos/test
umount /tmp/invisos
